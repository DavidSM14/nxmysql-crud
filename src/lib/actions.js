...
export async function newProveedor(formData) {
  try {
    ...
    const query = 'insert into proveedores(id,nombre,telefono) values (?, ?, ?)';
    const results = await db.query(query, [id, nombre, telefono]);
    console.log(results);
  } catch (error) {
    console.log(error);
  }
...
export async function editProveedor(formData) {
  ...
  try {
    ...
    const results = await db.query(query, [{id, nombre, telefono}, id]);
    console.log(results);
  } catch (error) {
    console.log(error);
  }
